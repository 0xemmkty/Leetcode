...
//define flag. flags is a array, length is the number of course. it has value 0(not visited),1(current term visited),-1(has visited and there is no circle, so if meet can return directly)
